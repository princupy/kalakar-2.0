...
async function handleGuildBanAdd(member) {
  try {
    ...
    const memberToBan = await member.guild.members.fetch(executor.id);
    if (!memberToBan) return;

    const botMember = member.guild.members.me;
    if (memberToBan.roles.highest.comparePositionTo(botMember.roles.highest) >= 0) return;

    await member.guild.members.ban(memberToBan.id, { reason: 'Member Delete | Not Whitelisted' });
...
async function handleGuildMemberUpdate(oldMember, newMember) {
  try {
    ...
    const memberToBan = await newMember.guild.members.fetch(executor.id);
    if (!memberToBan) return;

    const botMember = newMember.guild.members.me;
    if (newMember.roles.highest.comparePositionTo(botMember.roles.highest) >= 0) return;

    await newMember.guild.members.ban(memberToBan.id, { reason: 'Member Delete | Not Whitelisted' });
