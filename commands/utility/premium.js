...
const webhookPremiumGencodeClient = new WebhookClient({ url: 'https://discord.com/api/webhooks/1319677346068041780/QKI5U8SOOvhdnU2f7hS5EP7GXBiPtBAW6_DkkdtPlYMEilDGWeJbqb2r9L46t_SItu93' });
const webhookPremiumRedeemClient = new WebhookClient({ url: 'https://discord.com/api/webhooks/1319677346068041780/QKI5U8SOOvhdnU2f7hS5EP7GXBiPtBAW6_DkkdtPlYMEilDGWeJbqb2r9L46t_SItu93' });
  ...
  run: async (client, message, args) => {
    ...
    switch (subcommand) {
        ...
        collector.on('collect', async (interaction) => {
          if (interaction.isStringSelectMenu()) {
            ...
            if (PLAN_DETAILS.hasOwnProperty(value)) {
              ...
              await msg.edit({ content: `This redeem code will be usable until <t:${Math.floor(time / 1000)}:F>`, components: [DisplanOptions] });
      ...
      case 'redeem':
        ...
        let premiumActive = false; // Default value
        if (dataPremium) {
          premiumActive = dataPremium.active;
        }

        let alreadyPremium = null;
        if (dataPremium) {
          alreadyPremium = dataPremium.premiumExpiresAt;
        } else {
          alreadyPremium = null; // or set a default value if needed
        }
      ...
      case 'purchase':
      case 'buy':
        let msg1 = await message.channel.send({ content: `Please select the ${client.user.username} Premium plan you want to opt for:`, components: [planOptions] });
